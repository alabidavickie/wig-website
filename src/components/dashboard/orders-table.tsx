  ...
  if (!orders || orders.length === 0) {
    return (
      <div className="w-full bg-[#141414] border border-[#2A2A2D] py-12 text-center rounded-sm">
        <p className="text-[12px] font-bold uppercase tracking-widest text-zinc-600">No past orders found</p>
      </div>
    );
  }
        ...
        <table className="w-full text-left border-collapse">
          <thead className="bg-[#1A1A1D]">
            <tr className="border-b border-[#2A2A2D]">
              <th className="py-4 px-6 text-[10px] font-bold text-zinc-500 uppercase tracking-widest whitespace-nowrap">Order ID</th>
              <th className="py-4 px-6 text-[10px] font-bold text-zinc-500 uppercase tracking-widest whitespace-nowrap">Date</th>
              <th className="py-4 px-6 text-[10px] font-bold text-zinc-500 uppercase tracking-widest whitespace-nowrap">Item Name</th>
              <th className="py-4 px-6 text-[10px] font-bold text-zinc-500 uppercase tracking-widest whitespace-nowrap">Status</th>
              <th className="py-4 px-6 text-[10px] font-bold text-zinc-500 uppercase tracking-widest text-right whitespace-nowrap">Total</th>
            </tr>
          </thead>
            ...
            {orders.map((order) => {
              ...
              return (
                <tr key={order.id} className="hover:bg-[#1A1A1D] transition-colors group">
                  ...
                  <td className="py-4 px-6 text-[13px] text-zinc-500">{dateStr}</td>
          ...
          return (
            <div key={order.id} className="bg-[#141414] border border-[#2A2A2D] p-6 space-y-4 rounded-sm">
              <div className="flex justify-between items-start border-b border-[#2A2A2D] pb-4">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-600 mb-1">Order ID</p>
              ...
              <div className="flex justify-between items-end">
                <div>
                  <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-600 mb-1">Date</p>
                  <p className="text-[12px] text-zinc-500">{dateStr}</p>
                </div>
                <div className="text-right">
              ...
              <div className="bg-[#1A1A1D] p-4 rounded-sm border border-[#2A2A2D]">
                <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-600 mb-2">Acquisitions</p>
                <p className="text-[11px] text-zinc-400 uppercase tracking-wide">
                  {order.order_items?.[0]?.product_name || "Premium Unit"} 
                  {order.order_items?.length > 1 && <span className="text-zinc-600 font-medium ml-1"> (+{order.order_items.length - 1} others)</span>}
                </p>
              </div>
            </div>
          );
