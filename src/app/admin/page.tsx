...
export default async function AdminDashboardPage() {
  ...
  return (
    <div className="space-y-6 sm:space-y-10 animate-in fade-in duration-500 pb-20 text-white">
      {/* Header */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-3">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-white uppercase tracking-[0.1em]">Dashboard</h1>
          <p className="text-zinc-400 mt-1 uppercase tracking-widest text-[10px] font-bold">Silk Haus Overview</p>
        </div>
        <div className="text-[9px] sm:text-[10px] font-bold uppercase tracking-widest text-zinc-400 bg-[#141414] px-3 sm:px-4 py-2 border border-[#2A2A2D] shadow-sm">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 sm:gap-6">
        {stats.map((stat) => (
          <div key={stat.label} className="bg-[#141414] p-4 sm:p-6 border border-[#2A2A2D] shadow-sm transition-all hover:border-[#D5A754]/30 group rounded-sm">
            <div className="flex justify-between items-start mb-3 sm:mb-4">
              <div className={`bg-[#2A2A2D] ${stat.color} p-2 sm:p-3 rounded-sm group-hover:scale-110 transition-transform border border-[#3F3F46]`}>
            ...
            <h3 className="text-zinc-400 text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.15em] sm:tracking-[0.2em] mb-1">{stat.label}</h3>
            <p className="text-lg sm:text-2xl font-bold tracking-tighter text-white break-all sm:break-normal">{stat.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-4 sm:gap-8">
        {/* Sales Chart Mockup */}
        <div className="bg-[#141414] p-4 sm:p-8 border border-[#2A2A2D] shadow-sm min-h-[300px] sm:min-h-[450px] flex flex-col rounded-sm">
          ...
          <div className="flex-1 flex items-end gap-1 sm:gap-3 px-1 sm:px-2 pb-2 border-b border-l border-[#2A2A2D]">
             {chartHeights.map((h, i) => (
               <div 
                 key={i} 
                 style={{ height: `${h}%` }} 
                 className={`flex-1 transition-all duration-700 ease-out ${i === new Date().getMonth() ? 'bg-[#D5A754]' : 'bg-[#1A1A1D]'} hover:bg-white relative group rounded-t-sm`}
               >
        ...
        <div className="bg-[#141414] p-4 sm:p-8 border border-[#2A2A2D] shadow-sm flex flex-col rounded-sm text-white">
          <div className="flex justify-between items-center mb-6 sm:mb-10">
          ...
          <div className="space-y-5 sm:space-y-8 flex-1">
              {products.slice(0, 5).map((product) => (
                <div key={product.id} className="flex gap-3 sm:gap-5 items-center group cursor-pointer">
                  <div className="w-10 h-12 sm:w-14 sm:h-[72px] bg-[#0A0A0A] border border-[#2A2A2D] flex-shrink-0 group-hover:border-[#D5A754] transition-colors overflow-hidden relative rounded-sm">
                    <Image 
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[9px] sm:text-[10px] text-zinc-400 uppercase italic mt-1 tracking-tighter">{product.category}</p>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-[11px] sm:text-[12px] font-bold tracking-tighter text-white">£{product.base_price}</p>
      ...
      <div className="bg-[#141414] border border-[#2A2A2D] shadow-sm overflow-hidden rounded-sm">
        <div className="p-4 sm:p-8 border-b border-[#2A2A2D] flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 bg-[#0A0A0A]/30">
          ...
          <Link href="/admin/orders" className="text-[9px] sm:text-[10px] font-bold uppercase tracking-widest text-zinc-400 hover:text-white transition-colors">View All</Link>
        </div>
