...
export default async function OrdersPage() {
  ...
  return ( 
    <div className="flex flex-col gap-10 w-full max-w-6xl mx-auto text-foreground bg-background p-8 font-sans min-h-screen">
      <div className="flex justify-between items-end mb-6 border-b border-gray-200 pb-6">
        <div>
          <h2 className="text-3xl font-serif text-foreground tracking-widest uppercase mb-2">Order History</h2>
          <p className="text-muted-foreground text-sm">Track shipments and review your Silk Haus acquisitions.</p>
        </div>
        <Button variant="outline" className="border-border bg-transparent text-foreground hover:bg-card text-xs h-10 px-6 rounded-none font-medium uppercase tracking-widest transition-colors">
          Help Center
        </Button>
      </div>
      ...
      <div className="space-y-12">
        ...
        {activeOrder ? (
          <div className="bg-card border border-border p-8 md:p-12 mb-16 relative overflow-hidden group">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12 relative z-10">
              <div>
                ...
                <div className="flex items-center gap-4">
                  <p className="font-serif italic text-3xl md:text-4xl text-foreground">Order #SH-{activeOrder.id.slice(0, 8)}</p>
                  <span className="bg-secondary border border-border px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-[#D5A754]">
                    {activeOrder.status}
                  </span>
                </div>
              </div>
                  ...
                  <div className="relative z-10 flex justify-between">
                    {[
                      ...
                      <div key={i} className="flex flex-col items-center gap-4 relative group cursor-default">
                        <div className={`w-12 h-12 rounded-full flex items-center justify-center border-2 transition-all duration-500 z-10 bg-background
                          ${step.done ? 'border-[#D5A754] text-black bg-[#D5A754]' : step.active ? 'border-[#D5A754] text-foreground shadow-[0_0_20px_rgba(213,167,84,0.1)]' : 'border-border text-muted-foreground/50'}`}
                        >
                        ...
                        <span className={`text-[10px] font-bold uppercase tracking-widest transition-colors duration-500 whitespace-nowrap
                          ${step.active || step.done ? 'text-foreground' : 'text-muted-foreground/50'}`}>
                          {step.label}
                        </span>
                      </div>
                    ))}
          ...
          <div className="bg-card border border-dashed border-border py-16 text-center mb-16 rounded-[24px]">
             <p className="text-[12px] font-bold uppercase tracking-widest text-muted-foreground/50">No active commissions</p>
          </div>
        )}
